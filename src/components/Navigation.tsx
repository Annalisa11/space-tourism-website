...
const Navigation = ({ isMenuOpen, toggleMenu }: Props) => {
  ...
  return (
    ...
    >
      <ul className="flex flex-col justify-between gap-12 md:flex-row">
        {navEntries.map(({ title, url }, index) => (
          <li key={index} className="md:flex md:items-center md:justify-center">
            <Link
              onClick={toggleMenu}
              className="flex h-full justify-center p-8 font-body text-lg uppercase tracking-wider text-white md:flex md:items-center md:border-b-4 md:border-transparent md:hover:cursor-pointer md:hover:border-white lg:text-3xl"
              to={url}
            >
